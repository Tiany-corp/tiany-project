...
const GamingStats = () => {
    ...
    return (
        <section
            ref={containerRef}
            className="w-screen relative min-h-screen overflow-hidden bg-gray-900 py-24 dark:bg-black"
        >
            {/* Darkmode toggle */}
            {/*<DarkModeToggle />*/}
                    ...
                    {stats.map((stat) => (
                        <motion.div
                            ...
                            className={`group relative overflow-hidden rounded-2xl border border-gray-700 bg-gray-800 p-6 shadow-lg dark:border-gray-800 dark:bg-gray-900 ${
                                hoveredIndex === stat.index ? "h-64" : "h-56"
                            }`} // <-- change la hauteur selon hover
                            layout="position" // <-- Framer Motion gère la transition douce de hauteur
                    ...
                    LEVEL
                            ...
                            <AnimatePresence>
                                {hoveredIndex === stat.index && (
                                    ...
                                    >
                                        <button
                                            className={`w-60 rounded-md bg-gradient-to-r py-1.5 px-4 text-xs font-medium text-white ${stat.color}`}
                                        >
                                            VIEW DETAILS
                                        </button>
                                    </motion.div>
